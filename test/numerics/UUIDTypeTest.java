...
public class UUIDTypeTest {
    ...
    @Test
    void testVersions6To15AreNotVersion5Either() {
        long propHighBits = version0HighBits();
        long lowBits = RANDOM.nextLong();
        long start = 6 * HIGH_BITS_VERSION_INCREMENT;
        for (long versionBits = start;
             versionBits < HIGH_BITS_VERSION_THRESHOLD;
             versionBits += HIGH_BITS_VERSION_INCREMENT) {
            long highBits = propHighBits + versionBits;
    ...
    @Test
    void testIsOfTypeMACVersion6() {
        long highBits = version0HighBits() + 6 * HIGH_BITS_VERSION_INCREMENT;
        long lowBits = RANDOM.nextLong();
        UUID uuid = new UUID(highBits, lowBits);
        String msg = "UUID " + uuid + " should be MAC, version 6";
    ...
    @Test
    void testVersions0To6AreNotVersion7() {
        long propHighBits = version0HighBits();
        long lowBits = RANDOM.nextLong();
        long stop = 7 * HIGH_BITS_VERSION_INCREMENT;
        for (long versionBits = 0L; versionBits < stop;
             versionBits += HIGH_BITS_VERSION_INCREMENT) {
            long highBits = propHighBits + versionBits;
            UUID uuid = new UUID(highBits, lowBits);
    ...
    @Test
    void testVersions1Through8AreKnown() {
        long propHighBits = version0HighBits();
        long lowBits = RANDOM.nextLong();
        long stop = 9 * HIGH_BITS_VERSION_INCREMENT;
        for (long versionBits = HIGH_BITS_VERSION_INCREMENT; versionBits < stop;
             versionBits += HIGH_BITS_VERSION_INCREMENT) {
            long highBits = propHighBits + versionBits;
            UUID uuid = new UUID(highBits, lowBits);
            String msg = uuid + " should not be unknown";
            assert !UUIDType.UNKNOWN.isOfType(uuid) : msg;
        }
    }
