...
public final class MandelbrotJuliaViewer extends JPanel
    ...
    private static final double DEFAULT_MANDEL_X_ADJUST = 0.75;

    private double mandelXAdjust = DEFAULT_MANDEL_X_ADJUST;

    private double xAdjust;

    private static final int ITERATION_MAXIMUM = 256;
    ...
    private ComplexNumber getNumber(int x, int y) {
        int adjustX = x - this.zeroCoordX;
        int adjustY = y - this.zeroCoordY;
        double re = 2.0 * (double) adjustX / this.zeroCoordX - this.xAdjust;
        double im = 1.25 * (double) adjustY / this.zeroCoordY;
        return new ComplexNumber(re, im);
    }

    private Color chooseColor(int iterationCount) {
        int adjustedCount = iterationCount % ITERATION_MAXIMUM;
    ...
    private Color iteratePointJulia(ComplexNumber c) {
        int iterCount = 0;
        ComplexNumber z = c;
        while (z.abs() < 2.0 && iterCount < ITERATION_MAXIMUM) {
    ...
    private void drawJulia(Graphics g) {
        ...
        for (int x = 0; x < this.maxX; x++) {
            for (int y = 0; y < this.zeroCoordY; y++) {
                curr = this.getNumber(x, y);
                color = this.iteratePointJulia(curr);
                g.setColor(color);
                g.drawRect(x, y, 1, 1);
                g.drawRect(this.maxX - x, this.maxY - y, 1, 1);
            }
        }
    }
    ...
    private void saveDiagramAs() {
        ...
        if (this.juliaFlag) {
            String pointStr = this.juliaPoint.toString().replace("\u2212", "n")
                    .replace(".", "d").replace("+", "p");
            suggestedFilename = "Julia Set " + pointStr + ".png";
        } else {
            suggestedFilename = "Mandelbrot Set.png";
        }
    ...
    private void toggleJuliaFlag() {
        ...
        if (this.juliaFlag) {
            ...
            this.xAdjust = this.mandelXAdjust;
        }
        this.repaint();
    }
    ...
    private void showAboutBox() {
        String title = "About";
        String msg = "Mandelbrot/Julia Set Viewer\nVersion 0.1\n"
                + "\u00A9 2021 Alonso del Arte";
        JOptionPane.showMessageDialog(this.frame, msg, title,
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        switch (cmd) {
            ...
            case "zoomIn":
//                this.zoomIn();
                System.err.println("Sorry, zoom in not available yet");
                break;
            case "zoomOut":
//                this.zoomOut();
                System.err.println("Sorry, zoom out not available yet");
                break;
...
//                this.showUserManual();
    ...
    private void setUpFrame() {
        ...
        this.zeroCoordX = this.maxX / 2;
        this.zeroCoordY = this.maxY / 2;
